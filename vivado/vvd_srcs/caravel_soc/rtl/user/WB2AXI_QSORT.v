...
always@(posedge wb_clk_i or posedge wb_rst_i)begin
    if(wb_rst_i)    axi_state <= IDLE;
        ...
        case(axi_state)
            IDLE:begin
                if(wb_valid)begin
                    if(wbs_adr_i == 32'h30000284)           
                        axi_state <= AXI_SM;
                    else if(wbs_adr_i == 32'h30000280)
                        axi_state <= AXI_SS;
                    else if(wbs_we_i)
                        axi_state <= AXI_WADDR;
                    else
...
assign araddr   = (axi_state == AXI_RADDR) ? wbs_adr_i - 32'h30000200 : 0;
assign arvalid  = (axi_state == AXI_RADDR) ? 1 : 0;
assign awaddr   = (axi_state == AXI_WADDR) ? wbs_adr_i - 32'h30000200 : 0;
